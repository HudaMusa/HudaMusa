...
public class TSPSolver {
	...
	public void run(ArrayList<Graph> G, int i, boolean suppressOutput) {
		//ask if boolean suppressOutput usage correct and how to incorporate ERROR: <name> did not find a TSP route for Graph <i>! when the algorithm <name> fails to find a TSP route for Graph i
	    Graph graph = G.get(i);
	    ...
	    while(visited.size() < graph.getN()) {
	        nextNode = nextNode(graph, visited);
	        if (nextNode[0] == -1) {
	        		System.out.print("\nError: " + getName() + " did not find a TSP route for Graph " + graphNum + "!");
		            return;
		    	}else {
	        visited.add(nextNode[1],nextNode[0]);
	        }
	    ...
	    if (!(graph.getArc(endNode-1,startNode-1))) {
	    	System.out.print("\nError: " + getName() + " did not find a TSP route for Graph " + graphNum + "!");
	        return;
	    	}
	    visited.add(visited.get(0));
	    visitedArray = visited.stream().mapToInt(Integer::intValue).toArray();

	    elapsedTime = (System.currentTimeMillis() - start); // Convert to seconds

	    // Set the results for this graph
	    setSolnPath(i, visitedArray);
	    setSolnCost(i, graph.pathCost(visitedArray));
	...
	public int nearestNeighbor(Graph G, ArrayList<Integer> visited, int k) { // Implementation for finding the nearest unvisited neighbor to the last visited node
		    ...
		    for (int i = 1; i <= G.getN(); i++) {
		        if ((!visited.contains(i)) && (G.getArc(k-1,i-1))) {
	...
	public void printSingleResult(int i, boolean rowOnly) {
	    ...
	    if (!solnFound[i]) {
	        System.out.printf("%3d           -                -   -    ", i + 1); //change formatting for space --> check whitespace online
	        System.out.println();
	    } else {
	        int[] path = solnPath.get(i);
	        System.out.printf("%3d%12.2f%17.3f", i + 1, solnCost[i], compTime[i]);
	        printPathWithDashes(path); 
	        System.out.println();
	    }
	}
